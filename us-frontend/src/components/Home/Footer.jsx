...
const Footer = () => {
  ...
  const data = [
    ...
    {
      id: 4,
      text: "✅ You are good to go with unityScript",
    },
  ];
        ...
        {data.map((item, i) => {
          return (
            <motion.div
              ...
              transition={{
                duration: 0.9,
                delay: i === 0 ? 0.25 : (i + 1) * 0.16,
              }}
              key={item.id}
              className="flex justify-start items-center gap-2 mt-3 bg-sub lg:py-4 xl:py-5 px-10 rounded-md"
            >
        ...
        <div className="connections flex justify-center gap-4 mt-6">
          <img
            ...
            onClick={() => {
              ...
              navigator.clipboard.writeText("+91 9549285813");
            }}
            alt=""
          />

          <Link to={"https://www.instagram.com/imsaurabh._/"} target="_blank">
            <img src={insta} className="w-4 cursor-pointer" alt="" />
          </Link>
          <Link to={"https://github.com/gitsumansaurabh"} target="_blank">
            <img src={github} className="w-4 cursor-pointer" alt="" />
          </Link>
        </div>
      </div>
    </div>
